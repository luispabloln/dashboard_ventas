...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="Master Sales Command v17.8", page_icon="💎", layout="wide")
...
def load_consolidated_data():
    ...
    if os.path.exists(VENTA_FILE):
        ...
        if df_v_raw is not None and 'fecha' in df_v_raw.columns:
            ...
            # Limpieza crítica de clienteId y cliente
...
def get_max_date_safe(df):
    if df is not None and not df.empty and 'fecha' in df.columns:
        # 1. Filtrar solo fechas válidas (ignorar NaT)
...
if df_v is not None:
    ...
    cobertura = dff['clienteid'].nunique() # AHORA USA EL ID DE CLIENTE (ÚNICO)
    ...
    with c2:
        ...
        k2.metric("Cobertura", f"{cobertura} Clientes") # ACTUALIZADO AQUÍ
    ...
    with tabs[2]:
        ...
        if not dff.empty:
            ...
            with c_m1:
                ...
                fig_combo.update_layout(yaxis2=dict(title="Cobertura (Clientes)", overlaying='y', side='right'), plot_bgcolor='white', height=400, title="Evolución Venta vs Clientes Únicos")
    ...
    with tabs[4]:
        ...
        if not dff.empty:
            ...
            with cc2:
                if 'cliente' in dff.columns and 'producto' in dff.columns and sel_cl:
            ...
            if 'cliente' in dff.columns and not dff['cliente'].empty:
                ...
                start_cl = set(dff[dff['fecha'] <= w1_end]['clienteid'].unique()) # USANDO ID
                end_cl = set(dff[dff['fecha'] >= w_last]['clienteid'].unique()) # USANDO ID
                churn_ids = list(start_cl - end_cl)
                
                # Para mostrar nombres, necesitamos mapear de nuevo, pero agruparemos por ID
