...
def load_consolidated_data():
    ...
    MAESTRO_FILE = 'Maestro_de_clientes.csv' # M mayúscula
    
    df_v, df_p, df_a = None, None, None
    
    def read_and_clean(file_path):
        # Buscar archivo ignorando mayúsculas si no existe exacto
    ...
    # 1. CARGAR VENTA
    df_v = read_and_clean(VENTA_FILE)
    if df_v is not None and 'fecha' in df_v.columns:
        ...
        # Canal Default (será sobrescrito si hay maestro)
        cat_map = {
    ...
    # 2. CARGAR MAESTRO
    df_a = read_and_clean(MAESTRO_FILE)
    if df_a is not None:
    ...
    # 3. CARGAR PREVENTA
    df_p = read_and_clean(PREVENTA_FILE)
    ...
    if df_v is not None and df_a is not None:
        ...
        # Recalcular canal con vendedor asignado
        df_v['canal'] = df_v['vendedor'].map(cat_map).fillna('6. RUTA TDB')
...
if df_v is not None:
    ...
    with c2:
        ...
        if df_p is not None:
            ...
            st.markdown(f'<div class="alert-box alert-warning">📉 Rechazos: ${caida:,.0f}</div>', unsafe_allow_html=True)
    ...
    with tabs[1]:
        if df_p is not None:
            ...
            c2.plotly_chart(px.bar(top_drop, x='caida', y='vendedor', orientation='h', title="Top Rechazos por Vendedor"), use_container_width=True)
    ...
    with tabs[5]:
        ...
        # Crear mapa Nombre -> ID para búsqueda
    ...
    # 7. AUDITORIA (CON FILTROS RESTAURADOS)
    with tabs[6]:
        st.header("🔍 Auditoría")
        ...
        if col_hm in df_aud.columns:
            ...
            st.plotly_chart(px.imshow(piv, aspect="auto", title=f"Mapa de Calor: Vendedor vs {col_hm}"), use_container_width=True)
