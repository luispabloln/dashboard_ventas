...
def load_consolidated_data():
    ...
    # Asegúrate de que en tu GitHub se llamen EXACTAMENTE así:
    VENTA_FILE = 'venta_completa.csv'
    PREVENTA_FILE = 'preventa_completa.csv'
    MAESTRO_FILE = 'Maestro_de_clientes.csv'  # <-- CORREGIDO: M mayúscula
    
    df_v, df_p, df_a = None, None, None
    
    def read_and_clean(file_path):
        # Intentar buscar el archivo con variaciones de nombre si no se encuentra exacto
        if not os.path.exists(file_path):
            # Intentar todo minúsculas
            if os.path.exists(file_path.lower()):
                file_path = file_path.lower()
            # Intentar capitalizado
            elif os.path.exists(file_path.capitalize()):
                file_path = file_path.capitalize()
                
        if os.path.exists(file_path):
            try:
                # Lectura robusta: primero ; luego ,
    ...
    # LECTURA DE ASIGNACIONES (MAESTRO)
    df_a_raw = read_and_clean(MAESTRO_FILE)
    if df_a_raw is not None:
        # Buscar columnas clave aunque tengan nombres ligeramente distintos
        col_id = next((c for c in df_a_raw.columns if 'cliente' in c and 'id' in c), None)
        col_vend = next((c for c in df_a_raw.columns if 'vendedor' in c), None)
...
# --- FUNCIÓN PARA OBTENER FECHA MÁXIMA ---
...
# --- INTERFAZ ---
with st.sidebar:
    st.title("💎 Master Dashboard v22.1")
    st.success("Sistema conectado a GitHub.")
    st.markdown("---")
    st.header("🎯 Metas")
...
if df_v is not None:
    ...
    with tabs[0]:
        if df_a is not None:
            ...
            st.error("⚠️ No se encontró el archivo 'Maestro_de_clientes.csv' (revisa las mayúsculas en GitHub).")

    # [RESTO DE PESTAÑAS IGUAL QUE V20.2]
    # (Por brevedad, se asume que copias el resto de las pestañas de la versión anterior, ya que funcionan bien)
    # ...
    ...
    with tabs[5]:
        ...
        with tabs[6]:
            ...
            if not dff.empty:
                ...
                if 'jerarquia1' in dff.columns: j1_opts = sorted(dff['jerarquia1'].dropna().unique())
                else: j1_opts = []
                if 'jerarquia2' in dff.columns: j2_opts = sorted(dff['jerarquia2'].dropna().unique())
                else: j2_opts = []
                if 'jerarquia3' in dff.columns: j3_opts = sorted(dff['jerarquia3'].dropna().unique())
                else: j3_opts = []
                if 'categoria' in dff.columns: cat_opts = sorted(dff['categoria'].dropna().unique())
                else: cat_opts = []
                if 'producto' in dff.columns: prod_opts = sorted(dff['producto'].dropna().unique())
                else: prod_opts = []
                ...
                if not df_audit.empty:
                    ...
                    if sel_prod: col_group = 'producto'
                    elif sel_cat: col_group = 'categoria'
                    elif sel_j3: col_group = 'jerarquia3'
                    elif sel_j2: col_group = 'jerarquia2'
                    elif sel_j1: col_group = 'jerarquia1'
                    else: col_group = 'jerarquia1' # Default
