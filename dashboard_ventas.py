...
if df_v is not None:
    ...
    # 4. ESTRATEGIA (CORREGIDO PARA MOSTRAR DATOS SIEMPRE)
    with tabs[3]:
        st.header("📈 Estrategia")
        
        # Agrupar por fecha para asegurar datos diarios únicos
        daily_data = dff.groupby('fecha').agg({
            'monto_real': 'sum',
            'clienteid': 'nunique'
        ...
        if not daily_data.empty:
            ...
            fig.add_trace(go.Bar(
                x=daily_data['fecha'], 
                y=daily_data['monto_real'], 
                name='Venta ($)', 
                marker_color='#95A5A6',
                opacity=0.6
            ))
            
            # Línea: Clientes (Cobertura)
            fig.add_trace(go.Scatter(
                x=daily_data['fecha'], 
                y=daily_data['clienteid'], 
                name='Cobertura (Clientes)', 
                yaxis='y2', 
                line=dict(color='#3498DB', width=3), 
                mode='lines+markers+text', # Mostrar puntos y texto
                text=daily_data['clienteid'], # Mostrar el número de clientes
            ...
            st.warning("No hay datos diarios disponibles para graficar.")
